...
public class JpegExifEntry {
	...
	public JpegExifEntry(byte[] tagNumber, int dataFormat, int componentCount, Object value, byte[] offset) {
	...
	public String getTagNumberAsString() {
		return String.format("%02x", (tagNumber[0] & 0xFF)) + String.format("%02x", (tagNumber[1] & 0xFF));
	}
	...
	public boolean equals(JpegExifEntry entry) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof JpegExifEntry) {
			//compare tagNumber
			return tagNumber[0] == ((JpegExifEntry) obj).getTagNumber()[0]
					&& tagNumber[1] == ((JpegExifEntry) obj).getTagNumber()[1];
		} else {
			return false;
		}
	}
	...
	public String toString() {
		...
		if (dataFormat == 5 || dataFormat == 10) {
			...
			for (int i = 0; i < castValue.length / 2; i++) {
				result += String.format("%d/%d:%.2f ", castValue[2 * i],
						castValue[2 * i + 1],
						(double) castValue[2 * i] / castValue[2 * i + 1]);
			}

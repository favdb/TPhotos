...
public class XmlPrintCell {
	...
	public XmlPrintCell(int id, int photo_id, String photo, String comment, int page, int... span) {
		...
		if (photo.startsWith(App.pref.photosDirGet())) {
			this.photoName = photo.replace(App.pref.photosDirGet() + File.separator, "");
		}
		this.type = "photo";
		this.page = page;
		this.spanSet(span);
	 ...
	 */
	@SuppressWarnings("OverridableMethodCallInConstructor")
	public XmlPrintCell(int id, int cellNum) {
		this.id = id;
		this.type = "";
	...
	public File photoFileGet() {
		...
		if (!f.exists()) {
			f = new File(App.pref.photosDirGet(), photoName);
		}
		return f;
	}
	...
	/**
	 * get the Xml String of this XmlPrintCell
	 *
	 * @return
	 */
	public String toXml() {
	...
	/**
	 * Clear content of this XmlPrintCell (without change of POSITION)
	 */
	public void clear() {
		this.type = "unknown";
		this.photoId = -1;
	...
	/**
	 * Swap content with the given XmlPrintCell (without change of POSITION)
	 *
	 * @param old
	 */
	public void swapContentWith(XmlPrintCell old) {
		if (old == null) {
			return;
		}
	 ...
	 * @param cells
	 */
	public static void sortByPage(List<XmlPrintCell> cells) {
		if (cells == null || cells.isEmpty()) {
			return;
		}

		cells.sort(Comparator.comparingInt(XmlPrintCell::pageGet)
				.thenComparingInt(XmlPrintCell::posNumGet)
		);
	}
	 ...
	 * @param cells
	 */
	public static void sortById(List<XmlPrintCell> cells) {
		if (cells == null || cells.isEmpty()) {
			return;
		}

		cells.sort(Comparator.comparingInt(XmlPrintCell::idGet)
				.thenComparingInt(XmlPrintCell::idGet)
		);
	}

	/**
	 * class to manage position data (cell number, span H and V) and zoom
	 */

...
public class Jpeg {
	...
	private static final String TT = "Jpeg.";
	...
	public Jpeg(File f) throws Exception {
		try (BufferedInputStream buff
			...
			if (buff.read(fileMarker) == -1) {
				buff.close();
				throw new Exception("Not have enough data on marker");
			}
			if (!((fileMarker[0] & 0xFF) == 0xFF
					&& (fileMarker[1] & 0xFF) == 0xD8)) {
				buff.close();
	...
	private byte[] readSegment(BufferedInputStream f) throws IOException {
		...
		if (f.read(header) == -1) {
			throw new IOException("Not enough data on reading segment header");
		}
		byte[] sizeData = new byte[2];
		if (f.read(sizeData) == -1) {
			throw new IOException("Not enough data on reading segment size");
		}

...
public class JpegExif {
	...
	private int position, gpsOffset = 0,
			subOffset = 0, ifd1Offset = 0,
			interoperabilityOffset = 0,
			imageWidth = 0, imageHeight = 0;
	private String latRef, longRef;
	private Integer latDegree = null,
			longDegree = null,
			latMinute = null,
			longMinute = null;
	...
	/**
	 * endian info represented by a boolean. If return value is true, exif is big endian.
	...
	private JpegExifEntry[] readIfd(byte[] exif) {
		...
		for (int i = 0; i < entryCount; i++) {
			...
			position += 4;
			Object value = getValue(offset,
					entryCollection[i].getDataFormat(),
					entryCollection[i].getComponentCount(),
					exif);
	...
	private void analyzeEntry(JpegExifEntry entry) {
		...
		if ((tagNumber[0] & 0xFF) == 0x88
				&& (tagNumber[1] & 0xFF) == 0x25) {
			gpsOffset = getObjectValue(entry.getValue());
		} else if ((tagNumber[0] & 0xFF) == 0x87
				&& (tagNumber[1] & 0xFF) == 0x69) {
	 ...
	 */
	private Object getValue(byte[] offset, int format,
			int componentCount, byte[] exif) {
		...
		switch (format) {
			...
			case 8:
				if (size == 2) {
					...
					shortValue[0] = value[0];
					shortValue[1] = value[1];
					return bigEndian ? Endian.Big.getSignedShort(shortValue)
							: Endian.Little.getSignedShort(shortValue);
				} else {
					...
					for (int i = 0; i < size / SHORT_SIZE; i++) {
						...
						shortValue[0] = value[2 * i];
						shortValue[1] = value[2 * i + 1];
						result[i] = bigEndian ? Endian.Big.getSignedShort(shortValue)
								: Endian.Little.getSignedShort(shortValue);
					}
					return result;
				}
	...
	private void analyzeSubIfd() {
		for (JpegExifEntry e : subIfd) {
			...
			if ((tagNumber[0] & 0xFF) == 0xa0
					&& (tagNumber[1] & 0xFF) == 0x02) {
				imageWidth = getObjectValue(e.getValue());
			} else if ((tagNumber[0] & 0xFF) == 0xa0
					&& (tagNumber[1] & 0xFF) == 0x03) {
				imageHeight = getObjectValue(e.getValue());
			} else if ((tagNumber[0] & 0xFF) == 0xa0
					&& (tagNumber[1] & 0xFF) == 0x05) {
	...
	private void analyzeGps() {
		for (JpegExifEntry e : gpsEntry) {
			byte[] tagNumber = e.getTagNumber();
			if ((tagNumber[0] & 0xFF) == 0x00
					&& (tagNumber[1] & 0xFF) == 0x01) {
				latRef = (String) e.getValue();
			} else if ((tagNumber[0] & 0xFF) == 0x00
					&& (tagNumber[1] & 0xFF) == 0x02) {
	...
	/**
	 * a signed int which 4 bytes represent.
	 */
	private int getInt32(byte[] value) {
		return bigEndian ? Endian.Big.getInt32(value)
				: Endian.Little.getInt32(value);
	}
	...
	private long getLong32(byte[] value) {
		return bigEndian ? Endian.Big.getLong32(value)
				: Endian.Little.getLong32(value);
	}
